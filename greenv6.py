...
def main():
    ...
    # --- Configuration for Speed/Quality ---
    # Experiment with these values to find the best balance for your needs.
    # Preset: 'ultrafast', 'superfast', 'veryfast', 'fast', 'medium', 'slow', 'slower', 'veryslow'
    # CRF: 0 (lossless) to 51 (worst quality), 23 is a common default. Higher = faster & lower quality.
    current_preset = "veryfast" # Change to 'superfast' or 'ultrafast' for more speed
    current_crf = 23             # Increase (e.g., to 28-32) for more speed and smaller files
    target_frame_rate = 30.0     # Ensure this matches the desired output frame rate

    print(f"\n🎲 Step 2: Generating a satisfying background video to match foreground duration (Preset: {current_preset}, CRF: {current_crf})...")
    clips_dir_path = os.path.join(script_dir, "Clips")
    if not os.path.exists(clips_dir_path):
        print(f"Error: 'Clips' directory not found at '{clips_dir_path}'. "
              "Please create this folder and place your background video clips inside it.")
        print("❌ Pipeline aborted: Cannot generate background video without clips.")
        return
    ...
    combine_videos_for_duration(
        ...
        frame_width=1080,
        frame_height=1920,
        frame_rate=target_frame_rate, # Pass target_frame_rate
        bitrate="6M",
    ...
    print(f"\n🧪 Step 3: Applying green screen effect and combining videos into final_output.mp4 (Preset: {current_preset}, CRF: {current_crf})...")
